...
namespace ProphetsWay.Utilities
{
	internal class HashWorker
	{
		...
		{
			Hasher.GenerateIncrementalHash(inputBuffer, bufferLength);
		}
	}

	internal static class HashAlgorithmExtensions
	{
		public static void GenerateIncrementalHash(this HashAlgorithm hasher, byte[] inputBuffer, int bufferLength)
		{
			if (bufferLength > 0)
